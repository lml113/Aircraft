...
{
	u8 status;
   	NRF24L01_CSN_OFF;                 //使能SPI传输
  	status =SPI1_ReadWriteByte(reg);//发送寄存器号
  	SPI1_ReadWriteByte(value);      //写入寄存器的值
  	NRF24L01_CSN_ON;                 //禁止SPI传输
  	return(status);       		    //返回状态值
}
//读取SPI寄存器值
//reg:要读的寄存器
...
void NRF24L01_RX_Mode(void)
{
	NRF24L01_CE_OFF;
  	...
  	NRF24L01_Write_Reg(NRF_WRITE_REG+RF_SETUP,0x0f);   //设置TX发射参数,7db增益,250K,低噪声增益开启
...
void NRF24L01_TX_Mode(void)
{
	...
	NRF24L01_Write_Reg(NRF_WRITE_REG+RF_SETUP,0x0f);		//设置TX发射参数,7db增益,250Kbps,低噪声增益开启
